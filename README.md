...
## Quick Docker Start
  ...
  ```bash
    ...
    source ~/.venvs/nsid/bin/activate
...
**NOTE**: *You can call your geonode project whatever you like **except 'geonode'**. Follow the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``nsid`` with whatever you would like to name your project.*
...
1. Generate the project
    ...
    mkvirtualenv --python=/usr/bin/python3 nsid
    pip install Django==3.2.16

    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile nsid
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./nsid/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./nsid/br/restore.sh $BKP_FOLDER_NAME
```
